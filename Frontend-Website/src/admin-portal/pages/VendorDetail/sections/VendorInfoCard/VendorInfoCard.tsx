...
export default function VendorInfoCard({ vendor, onStatusChange, onDelete, updatingStatus }: VendorInfoCardProps) {
  return (
    <div className="flex flex-col gap-5 rounded-lg border border-[#e2e8f0] bg-white p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-display text-h5 text-[#0a192f]">{vendor.name}</h2>
          <p className="text-sm text-[#64748b]">{vendor.sellerId}</p>
        </div>
        <div className="flex items-center gap-2">
          <a
            href={`/admin-portal/vendors/${vendor.id}/edit`}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-sm text-[#0f172a]">{vendor.email}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs font-semibold text-[#64748b]">Phone</span>
          <span className="text-sm text-[#0f172a]">{vendor.phone ?? '—'}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs font-semibold text-[#64748b]">Location</span>
          <span className="text-sm text-[#0f172a]">{vendor.location ?? '—'}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs font-semibold text-[#64748b]">Joined</span>
          <span className="text-sm text-[#0f172a]">{formatDate(vendor.joiningDate)}</span>
        </div>
      </div>
      ...
      <label className="flex flex-col gap-1.5">
        ...
        <select
          ...
          onChange={(event) => onStatusChange(event.target.value as VendorStatus)}
