...
import type { ApiBuyer, BuyerStatus } from '../../../../lib/api'
...
type BuyersTableProps = {
  buyers: ApiBuyer[]
  onDelete: (buyer: ApiBuyer) => void
}

const tabs: { label: string; status: BuyerStatus | 'All' }[] = [
  { label: 'All', status: 'All' },
  { label: 'New', status: 'NEW' },
  { label: 'Contacted', status: 'CONTACTED' },
  { label: 'Viewing Booked', status: 'VIEWING_BOOKED' },
  { label: 'Won', status: 'WON' },
  { label: 'Lost', status: 'LOST' },
]

const statusLabels: Record<BuyerStatus, string> = {
  NEW: 'New',
  CONTACTED: 'Contacted',
...
const statusTones: Record<BuyerStatus, BadgeTone> = {
  NEW: 'info',
  CONTACTED: 'progress',
  VIEWING_BOOKED: 'special',
  WON: 'success',
  LOST: 'danger',
}
...
export default function BuyersTable({ buyers, onDelete }: BuyersTableProps) {
  const [activeTab, setActiveTab] = useState<BuyerStatus | 'All'>('All')
  ...
  const filtered = useMemo(() => {
    ...
    return buyers.filter((buyer) => {
      ...
      const fullName = `${buyer.firstName} ${buyer.surname}`.toLowerCase()
      const matchesSearch = query === '' || fullName.includes(query) || buyer.email.toLowerCase().includes(query)
      return matchesTab && matchesSearch
    })
  }, [buyers, activeTab, search])
  ...
  function selectTab(status: BuyerStatus | 'All') {
    setActiveTab(status)
    setPage(1)
  }
    ...
    <div className="w-full overflow-hidden rounded-lg border border-[#e5e7eb] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#f3f4f6] px-5 py-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          />
          <ActionButton href="/admin-portal/buyers/new" label="Add New Buyer" variant="primary" icon={PlusIcon} />
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[56rem] border-collapse">
          ...
          <tbody>
            {paginated.map((buyer) => (
              <tr key={buyer.id} className="border-t border-[#f3f4f6]">
                <td className="px-5 py-2.5 font-display text-sm text-[#0a192f]">
                  {buyer.firstName} {buyer.surname}
                </td>
                <td className="px-5 py-2.5 text-xs text-[#64748b]">{buyer.email}</td>
                <td className="px-5 py-2.5 text-xs text-[#64748b]">{buyer.phone ?? '—'}</td>
                <td className="px-5 py-2.5 text-xs text-[#64748b]">{buyer.source}</td>
                <td className="px-5 py-2.5">
                  <StatusBadge label={statusLabels[buyer.status]} tone={statusTones[buyer.status]} />
                </td>
                <td className="px-5 py-2.5 text-xs text-[#64748b]">{formatDate(buyer.createdAt)}</td>
                <td className="px-5 py-2.5 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <ActionButton href={`/admin-portal/buyers/${buyer.id}`} label="View Details" icon={EyeIcon} />
                    <ActionButton label="Delete" variant="delete" icon={TrashIcon} onClick={() => onDelete(buyer)} />
                  </div>
                </td>
              </tr>
            ))}
      ...
      {filtered.length > 0 && (
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[#e5eaf0] px-6 py-4">
          <p className="text-sm text-[#64748b]">
            ...
            of <span className="font-semibold text-[#0f172a]">{filtered.length}</span> buyers
          </p>
