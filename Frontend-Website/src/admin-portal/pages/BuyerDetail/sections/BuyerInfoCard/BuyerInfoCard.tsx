...
type BuyerInfoCardProps = {
  buyer: ApiBuyer
  onStatusChange: (status: BuyerStatus) => void
  onDelete: () => void
  updatingStatus: boolean
}

const statusOptions: { value: BuyerStatus; label: string }[] = [
  { value: 'NEW', label: 'New' },
...
export default function BuyerInfoCard({ buyer, onStatusChange, onDelete, updatingStatus }: BuyerInfoCardProps) {
  return (
    <div className="flex flex-col gap-5 rounded-lg border border-[#e2e8f0] bg-white p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#64748b]">{buyer.buyerId}</p>
        </div>
        <div className="flex items-center gap-2">
          <a
            href={`/admin-portal/buyers/${buyer.id}/edit`}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-sm text-[#0f172a]">{buyer.email}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs font-semibold text-[#64748b]">Phone</span>
          <span className="text-sm text-[#0f172a]">{buyer.phone ?? '—'}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs font-semibold text-[#64748b]">Source</span>
          <span className="text-sm text-[#0f172a]">{buyer.source}</span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-xs font-semibold text-[#64748b]">Added</span>
          <span className="text-sm text-[#0f172a]">{formatDate(buyer.createdAt)}</span>
        </div>
      </div>
      ...
      <label className="flex flex-col gap-1.5">
        ...
        <select
          ...
          onChange={(event) => onStatusChange(event.target.value as BuyerStatus)}
