        ...
        <div className="w-[90%] sm:w-[80%] md:w-[65%] lg:w-[clamp(16rem,22%,19rem)] lg:shrink-0">
          <div className="flex flex-col gap-6">
            {left.map((reason) => (
              <article
                key={reason.title}
                className="flex flex-1 flex-col items-start justify-center gap-3 rounded-lg bg-white p-5 short:p-3 text-left lg:items-center lg:text-center"
              >
                <span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-navy-darkest lg:size-14">
                  <img src={reason.icon} alt="" aria-hidden="true" className="size-[22px] object-contain lg:size-7" />
                </span>
                <h3 className="font-display text-h6 text-ink capitalize">
                  {reason.title}
                </h3>
                <p className="text-body-sm text-text-body">{reason.description}</p>
              </article>
            ))}
          </div>
        </div>
